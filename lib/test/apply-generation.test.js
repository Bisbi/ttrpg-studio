...
describe("applyGeneration", () => {
  ...
  it("preserva il testo dopo la chiusura del blocco relations", () => {
    const settingPath = settingDir();
    // _relations.md con testo DOPO il blocco
    writeFileSync(
      join(settingPath, "_relations.md"),
      "# Relazioni\n\n```relations\n```\n\n## Legenda\nostile = guerra aperta\n"
    );
    applyGeneration({
      settingPath,
      element: { kind: "faction", name: "Lega", markdown: "# Lega",
        edges: [{ source: "lega", type: "ostile", target: "corte" }] },
    });
    const rel = readFileSync(join(settingPath, "_relations.md"), "utf8");
    expect(rel).toContain("lega --ostile--> corte");
    expect(rel).toContain("## Legenda");          // suffisso preservato
    expect(rel).toContain("ostile = guerra aperta");
  });
